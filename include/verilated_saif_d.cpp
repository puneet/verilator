...
// Construct the dump. Optional argument is ignored
ESDLVerilatedSaifD::ESDLVerilatedSaifD(void* filep) {
  m_saif_c = new VerilatedSaifC(filep);
}
// Destruct, flush, and close the dump
...
// Users should not need to call this, as for Verilated models, these
// propagate from the Verilated default timeunit
void ESDLVerilatedSaifD::set_time_unit(const char* unit) VL_MT_SAFE { m_saif_c->set_time_unit(unit); }
void ESDLVerilatedSaifD::set_time_unit(const std::string& unit) VL_MT_SAFE { m_saif_c->set_time_unit(unit); }
// Set time resolution (s/ms, defaults to ns)
// Users should not need to call this, as for Verilated models, these
// propagate from the Verilated default timeprecision
void ESDLVerilatedSaifD::set_time_resolution(const char* unit) VL_MT_SAFE { m_saif_c->set_time_resolution(unit); }
void ESDLVerilatedSaifD::set_time_resolution(const std::string& unit) VL_MT_SAFE { m_saif_c->set_time_resolution(unit); }
// Set variables to dump, using $dumpvars format
// If level = 0, dump everything and hier is then ignored
void ESDLVerilatedSaifD::dumpvars(int level, const std::string& hier) VL_MT_SAFE { m_saif_c->dumpvars(level, hier); }
