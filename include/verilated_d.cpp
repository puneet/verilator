...
void debugLevel(int level) VL_MT_SAFE { Verilated::debug(level); }
...
void lastContextp(VerilatedContext* contextp) VL_MT_SAFE { Verilated::lastContextp(contextp); }
/// Return the last VerilatedContext accessed
/// Generally threadContextp() should be called instead
VerilatedContext* lastContextp() VL_MT_SAFE { return Verilated::lastContextp(); }
...
void threadContextp(VerilatedContext* contextp) VL_MT_SAFE { Verilated::threadContextp(contextp); }
/// Return the VerilatedContext for the current thread
VerilatedContext* threadContextp() VL_MT_SAFE { return Verilated::threadContextp(); }
/// Return the global VerilatedContext, used if none created by user
VerilatedContext* defaultContextp() VL_MT_SAFE { return Verilated::defaultContextp(); }

/// Call VerilatedContext::assertOn using current thread's VerilatedContext
void assertOn(bool flag) VL_MT_SAFE { Verilated::threadContextp()->assertOn(flag); }
/// Return VerilatedContext::assertOn() using current thread's VerilatedContext
bool assertOn() VL_MT_SAFE { return Verilated::threadContextp()->assertOn(); }
/// Call VerilatedContext::calcUnusedSigs using current thread's VerilatedContext
void calcUnusedSigs(bool flag) VL_MT_SAFE { Verilated::calcUnusedSigs(flag); }
/// Return VerilatedContext::calcUnusedSigs using current thread's VerilatedContext
bool calcUnusedSigs() VL_MT_SAFE { return Verilated::calcUnusedSigs(); }
/// Call VerilatedContext::commandArgs using current thread's VerilatedContext
void commandArgs(int argc, const char** argv) VL_MT_SAFE { Verilated::commandArgs(argc, argv); }
void commandArgs(int argc, char** argv) VL_MT_SAFE { Verilated::commandArgs(argc, argv); }
/// Call VerilatedContext::commandArgsAdd using current thread's VerilatedContext
void commandArgsAdd(int argc, const char** argv) { Verilated::commandArgsAdd(argc, argv); }
...
void overWidthError(const char* signame) VL_MT_SAFE { Verilated::overWidthError(signame); }
void scTimePrecisionError(int sc_prec, int vl_prec) VL_MT_SAFE {
    Verilated::scTimePrecisionError(sc_prec, vl_prec);
}
void scTraceBeforeElaborationError() VL_MT_SAFE { Verilated::scTraceBeforeElaborationError(); }

// Internal: Get and set DPI context
const VerilatedScope* dpiScope() VL_MT_SAFE { return Verilated::dpiScope(); }
void dpiScope(const VerilatedScope* scopep) VL_MT_SAFE { Verilated::dpiScope(scopep); }
void dpiContext(const VerilatedScope* scopep, const char* filenamep, int lineno) VL_MT_SAFE {
    Verilated::dpiContext(scopep, filenamep, lineno);
}
void dpiClearContext() VL_MT_SAFE { Verilated::dpiClearContext(); }
bool dpiInContext() VL_MT_SAFE { return Verilated::dpiInContext(); }
const char* dpiFilenamep() VL_MT_SAFE { return Verilated::dpiFilenamep(); }
int dpiLineno() VL_MT_SAFE { return Verilated::dpiLineno(); }
int exportFuncNum(const char* namep) VL_MT_SAFE { return Verilated::exportFuncNum(namep); }

// Internal: Set the mtaskId, called when an mtask starts
// Per thread, so no need to be in VerilatedContext
void mtaskId(uint32_t id) VL_MT_SAFE { Verilated::mtaskId(id); }
uint32_t mtaskId() VL_MT_SAFE { return Verilated::mtaskId(); }
void endOfEvalReqdInc() VL_MT_SAFE { Verilated::endOfEvalReqdInc(); }
void endOfEvalReqdDec() VL_MT_SAFE { Verilated::endOfEvalReqdDec(); }
...
void endOfEval(VerilatedEvalMsgQueue* evalMsgQp) VL_MT_SAFE { Verilated::endOfEval(evalMsgQp); }
}  //namespace verilated
